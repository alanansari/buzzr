...
const QuestionAndResult = (params: {
    ...
    return (
        <>
            {params.screen === "question" && <div style={{
                width: `${percent}%`,
            ...
            <div className="w-full h-full flex gap-4 md:py-4 md:px-8 [&>*]:bg-white dark:[&>*]:bg-dark md:[&>*]:rounded-xl">
                ...
                {params.screen === "question" ? <div className="w-full p-6 flex flex-col min-h-full h-fit">
                    ...
                    <p className="dark:text-white mb-1 md:hidden font-bold text-center my-6 text-lg">Room code: {params.gameCode}</p>
                    ...
                    <div className="flex flex-col justify-center items-center">
                        ...
                        />
                        <p className={`text-xl xl:text-3xl font-medium mt-2 ${params.status === "correct" ? "text-[#20A97C]"
                            : params.status === "incorrect" ? "text-red-dark" : "text-[#F2AB53]"}`}>{params.message}</p>
                    </div>
                </div>}
            </div>
        </>
