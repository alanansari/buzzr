...
export default function GridListToggle() {
    ...
    return <>
        <div className="grid grid-cols-2 bg-card-light dark:bg-[#332D40] rounded h-fit p-2 gap-x-2">
            <div className={` cursor-pointer flex gap-x-1 items-center rounded p-2 ${view === "grid" && "bg-white dark:bg-[#27272A]"}`} onClick={() => { handleToggle({ view: "grid" }) }}>
                <IoGridOutline className="dark:text-white" />
                <span className="dark:text-white hidden md:inline">Grid</span>
            </div>

            <div className={` cursor-pointer flex gap-x-1 items-center p-2 rounded ${view === "list" && "bg-white dark:bg-[#27272A]"}`} onClick={() => { handleToggle({ view: "list" }) }}>
